...
public class BinarySearchAutocomplete implements Autocompletor {
	 ...
	 */
	/**
	create a priority queue full of the highest-weighted terms that match the prefix
	 */
	@Override
	public List<Term> topMatches(String prefix, int k) {
		...
		Term[] firstlast = Arrays.copyOfRange(myTerms, first, last+1);
		List <Term> list = Arrays.asList(firstlast);
	...
	public void initialize(String[] terms, double[] weights) {
		if (terms.length < 0) {
			throw new IllegalArgumentException("Illegal term value:");
		}
